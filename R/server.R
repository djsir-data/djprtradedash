...
server <- function(input, output, session) {
  ...
  merch_explorer_plot <- reactive({
    req(input$merch_explorer_dates,
        input$merch_countries,
        input$merch_sitc,
        input$merch_explorer_facets)

    merch %>%
      dplyr::filter(.data$date >= input$merch_explorer_dates[1],
                    .data$date <= input$merch_explorer_dates[2]) %>%
      viz_merch_explorer(countries = input$merch_countries,
  ...
  djprshiny::download_server(id = "merch_explorer_dl",
